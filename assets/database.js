(function () {
  ...
  function invoiceFromRow(r) {
    return {
      id: String(r.id), invoiceNo: String(r.invoice_number||'').startsWith('__NO_INVOICE__')?'':r.invoice_number, invoiceDate: r.invoice_date, dueDate: r.due_date,
  ...
  async function getNextInvoiceNumber(invoiceDate) {
    const { data, error } = await db().rpc('ei_next_invoice_number', {
      p_workspace_id: ws(),
  ...
  async function saveInvoice(inv) {
    const header = {
      workspace_id: ws(), invoice_number: inv.invoiceNo || `__NO_INVOICE__${inv.id||Date.now()}_${Math.random().toString(36).slice(2,8)}`, invoice_date: inv.invoiceDate, due_date: inv.dueDate || null,
  ...
  window.EasyDB = { configured, listCustomers, saveCustomer, deleteCustomer, listMaterials, saveMaterial, deleteMaterial, listInvoices, getNextInvoiceNumber, saveInvoice, importLegacy };
})();
